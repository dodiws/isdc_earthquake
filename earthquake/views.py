...
import urllib3
import shutil
from geodb.enumerations import EARTHQUAKE_TYPES

def get_dashboard_meta(page_name):
    if page_name == 'earthquake':
        return {'function':dashboard_earthquake, 'template':'dash_earthquake.html'}
    return None
...
def dashboard_earthquake(request, filterLock, flag, code, includes=[], excludes=[]):
	response = dict_ext(getCommonUse(request, flag, code))
	response['source'] = getEarthquake(request, filterLock, flag, code)
